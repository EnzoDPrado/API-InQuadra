...
const CollectionTypeFormWrapper = ({ allLayoutData, children, slug, id, origin }) => {
  ...
  const onPost = (0,react.useCallback)(
    async (body, trackerProperty) => {
      const endPoint = typeof origin === "string" ? `${(0,utils/* getRequestUrl */.IF)(`collection-types/${slug}/clone/${origin}`)}${rawQuery}` : `${(0,utils/* getRequestUrl */.IF)(`collection-types/${slug}`)}${rawQuery}`;
      try {
...
function useSelect({
  ...
  const relationFetchEndpoint = (0,react.useMemo)(() => {
    ...
    return (0,utils/* getRequestUrl */.IF)(`relations/${slug}/${entityId}/${name.split(".").at(-1)}`);
...
const Option_Option = ({ publicationState, mainField, id }) => {
  ...
  if (publicationState) {
    ...
    return /* @__PURE__ */ react.createElement(ComboboxOption/* ComboboxOption */.O, { value: id, textValue: mainField ?? id }, /* @__PURE__ */ react.createElement(Flex/* Flex */.k, null, /* @__PURE__ */ react.createElement(StyledBullet, { title, isDraft }), /* @__PURE__ */ react.createElement(Typography/* Typography */.Z, { ellipsis: true }, mainField ?? id)));
  }
  return /* @__PURE__ */ react.createElement(ComboboxOption/* ComboboxOption */.O, { value: id, textValue: mainField ?? id }, mainField ?? id);
};
...
var InformationBox=function(){if(window&&window.strapi&&window.strapi.isEE){return (__webpack_require__(18274)/* ["default"] */ .Z);}return (__webpack_require__(13164)/* ["default"] */ .Z);}();// TODO: this seems suspicious
var CTB_PERMISSIONS=[{action:'plugin::content-type-builder.read',subject:null}];/* eslint-disable  react/no-array-index-key */var EditView=function EditView(_ref){var allowedActions=_ref.allowedActions,isSingleType=_ref.isSingleType,goBack=_ref.goBack,slug=_ref.slug,id=_ref.id,origin=_ref.origin,userPermissions=_ref.userPermissions;var _useTracking=(0,helper_plugin_esm/* useTracking */.rS)(),trackUsage=_useTracking.trackUsage;var _useIntl=(0,useIntl/* default */.Z)(),formatMessage=_useIntl.formatMessage;var permissions=(0,es/* useSelector */.v9)(selectors/* selectAdminPermissions */._);var location=(0,react_router/* useLocation */.TH)();var toggleNotification=(0,helper_plugin_esm/* useNotification */.lm)();useOnce(function(){/**
     * We only ever want to fire the notification once otherwise
     * whenever the app re-renders it'll pop up regardless of
     * what we do because the state comes from react-router-dom
     */if(location!==null&&location!==void 0&&location.state&&'error'in location.state){toggleNotification({type:'warning',message:location.state.error,timeout:5000});}});var _useSelector=(0,es/* useSelector */.v9)(function(state){return{layout:selectCurrentLayout(state),formattedContentTypeLayout:selectAttributesLayout(state),customFieldUids:selectCustomFieldUids(state)};}),layout=_useSelector.layout,formattedContentTypeLayout=_useSelector.formattedContentTypeLayout,customFieldUids=_useSelector.customFieldUids;var _useLazyComponents=hooks_useLazyComponents(customFieldUids),isLazyLoading=_useLazyComponents.isLazyLoading,lazyComponentStore=_useLazyComponents.lazyComponentStore;var _getFieldsActionMatch=utils_getFieldsActionMatchingPermissions(userPermissions,slug),createActionAllowedFields=_getFieldsActionMatch.createActionAllowedFields,readActionAllowedFields=_getFieldsActionMatch.readActionAllowedFields,updateActionAllowedFields=_getFieldsActionMatch.updateActionAllowedFields;var configurationPermissions=isSingleType?permissions.contentManager.singleTypesConfigurations:permissions.contentManager.collectionTypesConfigurations;// // FIXME when changing the routing
...
;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/pages/ListView/components/ConfirmDialogDeleteAll/index.js
...
const ConfirmDialogDeleteAll_ConfirmDialogDeleteAll = ({
  isConfirmButtonLoading,
  isOpen,
  onToggleDialog,
  onConfirm
...
ConfirmDialogDeleteAll_ConfirmDialogDeleteAll.propTypes = {
  isConfirmButtonLoading: (prop_types_default()).bool.isRequired,
  isOpen: (prop_types_default()).bool.isRequired,
  onConfirm: (prop_types_default()).func.isRequired,
  onToggleDialog: (prop_types_default()).func.isRequired
...
;// CONCATENATED MODULE: ./.cache/admin/src/content-manager/pages/ListView/components/TableRows/index.js
...
const TableRows = ({
  ...
  return /* @__PURE__ */ react.createElement(Tbody/* Tbody */.p, null, rows.map((data, index) => {
    ...
    return /* @__PURE__ */ react.createElement(
      ...
      withMainAction && /* @__PURE__ */ react.createElement(Cell.Td, { onClick: (e) => e.stopPropagation() }, /* @__PURE__ */ react.createElement(
      ...
      withBulkActions && /* @__PURE__ */ react.createElement(Cell.Td, null, /* @__PURE__ */ react.createElement(Flex/* Flex */.k, { as: "span", justifyContent: "end", gap: 1, onClick: (e) => e.stopPropagation() }, /* @__PURE__ */ react.createElement(
        IconButton/* IconButton */.h,
        {
...
function ownKeys(object,enumerableOnly){var keys=Object.keys(object);if(Object.getOwnPropertySymbols){var symbols=Object.getOwnPropertySymbols(object);enumerableOnly&&(symbols=symbols.filter(function(sym){return Object.getOwnPropertyDescriptor(object,sym).enumerable;})),keys.push.apply(keys,symbols);}return keys;}function _objectSpread(target){for(var i=1;i<arguments.length;i++){var source=null!=arguments[i]?arguments[i]:{};i%2?ownKeys(Object(source),!0).forEach(function(key){(0,defineProperty/* default */.Z)(target,key,source[key]);}):Object.getOwnPropertyDescriptors?Object.defineProperties(target,Object.getOwnPropertyDescriptors(source)):ownKeys(Object(source)).forEach(function(key){Object.defineProperty(target,key,Object.getOwnPropertyDescriptor(source,key));});}return target;}var getReviewWorkflowsColumn=function(){if(window&&window.strapi&&window.strapi.isEE){return (__webpack_require__(49179)/* ["default"] */ .Z);}return (__webpack_require__(43973)/* ["default"] */ .Z);}();var ConfigureLayoutBox=/*#__PURE__*/(0,styled_components_browser_esm/* default */.ZP)(Box/* Box */.x).withConfig({displayName:"ListView__ConfigureLayoutBox",componentId:"sc-1owh71w-0"})(["svg{path{fill:",";}}"],function(_ref){var theme=_ref.theme;return theme.colors.neutral900;});function ListView(_ref2){var canCreate=_ref2.canCreate,canDelete=_ref2.canDelete,canRead=_ref2.canRead,canPublish=_ref2.canPublish,data=_ref2.data,getData=_ref2.getData,getDataSucceeded=_ref2.getDataSucceeded,isLoading=_ref2.isLoading,layout=_ref2.layout,pagination=_ref2.pagination,slug=_ref2.slug;var total=pagination.total;var contentType=layout.contentType;var info=contentType.info,options=contentType.options,metadatas=contentType.metadatas,_contentType$settings=contentType.settings,isBulkable=_contentType$settings.bulkable,isFilterable=_contentType$settings.filterable,isSearchable=_contentType$settings.searchable;var toggleNotification=(0,helper_plugin_esm/* useNotification */.lm)();var _useTracking=(0,helper_plugin_esm/* useTracking */.rS)(),trackUsage=_useTracking.trackUsage;var _useRBACProvider=(0,helper_plugin_esm/* useRBACProvider */.vn)(),refetchPermissions=_useRBACProvider.refetchPermissions;var trackUsageRef=react.useRef(trackUsage);var fetchPermissionsRef=react.useRef(refetchPermissions);var _useNotifyAT=(0,useNotifyAT/* useNotifyAT */.G)(),notifyStatus=_useNotifyAT.notifyStatus;var _useAPIErrorHandler=(0,helper_plugin_esm/* useAPIErrorHandler */.So)(utils/* getTrad */.OB),formatAPIError=_useAPIErrorHandler.formatAPIError;var permissions=(0,es/* useSelector */.v9)(selectors/* selectAdminPermissions */._);(0,helper_plugin_esm/* useFocusWhenNavigate */.go)();var _useQueryParams=(0,helper_plugin_esm/* useQueryParams */.Kx)(),_useQueryParams2=(0,slicedToArray/* default */.Z)(_useQueryParams,1),query=_useQueryParams2[0].query;var params=utils_buildQueryString(query);var pluginsQueryParams=(0,qs_lib.stringify)({plugins:query.plugins},{encode:false});var _useLocation=(0,react_router/* useLocation */.TH)(),pathname=_useLocation.pathname;var _useHistory=(0,react_router/* useHistory */.k6)(),push=_useHistory.push;var _useIntl=(0,useIntl/* default */.Z)(),formatMessage=_useIntl.formatMessage;var hasDraftAndPublish=(options===null||options===void 0?void 0:options.draftAndPublish)||false;var fetchClient=(0,helper_plugin_esm/* useFetchClient */.kY)();var post=fetchClient.post,del=fetchClient.del;var bulkPublishMutation=(0,react_query_es.useMutation)(function(data){return post("/content-manager/collection-types/".concat(contentType.uid,"/actions/bulkPublish"),data);},{onSuccess:function onSuccess(){toggleNotification({type:'success',message:{id:'content-manager.success.record.publish',defaultMessage:'Published'}});fetchData("/content-manager/collection-types/".concat(slug).concat(params));},onError:function onError(error){toggleNotification({type:'warning',message:formatAPIError(error)});}});var bulkUnpublishMutation=(0,react_query_es.useMutation)(function(data){return post("/content-manager/collection-types/".concat(contentType.uid,"/actions/bulkUnpublish"),data);},{onSuccess:function onSuccess(){toggleNotification({type:'success',message:{id:'content-manager.success.record.unpublish',defaultMessage:'Unpublished'}});fetchData("/content-manager/collection-types/".concat(slug).concat(params));},onError:function onError(error){toggleNotification({type:'warning',message:formatAPIError(error)});}});// FIXME
// Using a ref to avoid requests being fired multiple times on slug on change
// We need it because the hook as mulitple dependencies so it may run before the permissions have checked
var requestUrlRef=react.useRef('');/**
   * TODO: re-write all of this, it's a mess.
   */var fetchData=react.useCallback(/*#__PURE__*/function(){var _ref3=(0,asyncToGenerator/* default */.Z)(/*#__PURE__*/regenerator_default().mark(function _callee(endPoint,source){var opts,_yield$fetchClient$ge,_yield$fetchClient$ge2,results,paginationResult,_err$response$status,_err$response,resStatus;return regenerator_default().wrap(function _callee$(_context){while(1)switch(_context.prev=_context.next){case 0:getData();_context.prev=1;opts=source?{cancelToken:source.token}:null;_context.next=5;return fetchClient.get(endPoint,opts);case 5:_yield$fetchClient$ge=_context.sent;_yield$fetchClient$ge2=_yield$fetchClient$ge.data;results=_yield$fetchClient$ge2.results;paginationResult=_yield$fetchClient$ge2.pagination;notifyStatus(formatMessage({id:(0,utils/* getTrad */.OB)('utils.data-loaded'),defaultMessage:'{number, plural, =1 {# entry has} other {# entries have}} successfully been loaded'},// Using the plural form
{number:paginationResult.count}));getDataSucceeded(paginationResult,results);_context.next=25;break;case 13:_context.prev=13;_context.t0=_context["catch"](1);if(!axios["default"].isCancel(_context.t0)){_context.next=17;break;}return _context.abrupt("return");case 17:resStatus=(_err$response$status=_context.t0===null||_context.t0===void 0||(_err$response=_context.t0.response)===null||_err$response===void 0?void 0:_err$response.status)!==null&&_err$response$status!==void 0?_err$response$status:null;if(!(resStatus===403)){_context.next=24;break;}_context.next=21;return fetchPermissionsRef.current();case 21:toggleNotification({type:'info',message:{id:(0,utils/* getTrad */.OB)('permissions.not-allowed.update')}});push('/');return _context.abrupt("return");case 24:toggleNotification({type:'warning',message:{id:(0,utils/* getTrad */.OB)('error.model.fetch')}});case 25:case"end":return _context.stop();}},_callee,null,[[1,13]]);}));return function(_x,_x2){return _ref3.apply(this,arguments);};}(),[formatMessage,getData,getDataSucceeded,notifyStatus,push,toggleNotification,fetchClient]);var handleConfirmDeleteAllData=react.useCallback(/*#__PURE__*/function(){var _ref4=(0,asyncToGenerator/* default */.Z)(/*#__PURE__*/regenerator_default().mark(function _callee2(ids){var requestUrl;return regenerator_default().wrap(function _callee2$(_context2){while(1)switch(_context2.prev=_context2.next){case 0:_context2.prev=0;_context2.next=3;return post((0,utils/* getRequestUrl */.IF)("collection-types/".concat(slug,"/actions/bulkDelete")),{ids:ids});case 3:requestUrl=(0,utils/* getRequestUrl */.IF)("collection-types/".concat(slug).concat(params));fetchData(requestUrl);trackUsageRef.current('didBulkDeleteEntries');_context2.next=11;break;case 8:_context2.prev=8;_context2.t0=_context2["catch"](0);toggleNotification({type:'warning',message:formatAPIError(_context2.t0)});case 11:case"end":return _context2.stop();}},_callee2,null,[[0,8]]);}));return function(_x3){return _ref4.apply(this,arguments);};}(),[fetchData,params,slug,toggleNotification,formatAPIError,post]);var handleConfirmDeleteData=react.useCallback(/*#__PURE__*/function(){var _ref5=(0,asyncToGenerator/* default */.Z)(/*#__PURE__*/regenerator_default().mark(function _callee3(idToDelete){var requestUrl;return regenerator_default().wrap(function _callee3$(_context3){while(1)switch(_context3.prev=_context3.next){case 0:_context3.prev=0;_context3.next=3;return del((0,utils/* getRequestUrl */.IF)("collection-types/".concat(slug,"/").concat(idToDelete)));case 3:requestUrl=(0,utils/* getRequestUrl */.IF)("collection-types/".concat(slug).concat(params));fetchData(requestUrl);toggleNotification({type:'success',message:{id:(0,utils/* getTrad */.OB)('success.record.delete')}});_context3.next=11;break;case 8:_context3.prev=8;_context3.t0=_context3["catch"](0);toggleNotification({type:'warning',message:formatAPIError(_context3.t0)});case 11:case"end":return _context3.stop();}},_callee3,null,[[0,8]]);}));return function(_x4){return _ref5.apply(this,arguments);};}(),[slug,params,fetchData,toggleNotification,formatAPIError,del]);/**
   ...
   */var validateEntriesToPublish=/*#__PURE__*/function(){var _ref6=(0,asyncToGenerator/* default */.Z)(/*#__PURE__*/regenerator_default().mark(function _callee4(selectedEntries){var validations,schema,entries,validationPromises,resolvedPromises;return regenerator_default().wrap(function _callee4$(_context4){while(1)switch(_context4.prev=_context4.next){case 0:validations={validIds:[],errors:{}};// Create the validation schema based on the contentType
schema=(0,utils/* createYupSchema */.Ex)(contentType,{components:layout.components},{isDraft:false});// Get the selected entries
entries=data.filter(function(entry){return selectedEntries.includes(entry.id);});// Validate each entry and map the unresolved promises
validationPromises=entries.map(function(entry){return schema.validate(entry,{abortEarly:false});});// Resolve all the promises in one go
_context4.next=6;return Promise.allSettled(validationPromises);case 6:resolvedPromises=_context4.sent;// Set the validations
resolvedPromises.forEach(function(promise){if(promise.status==='rejected'){var entityId=promise.reason.value.id;validations.errors[entityId]=(0,helper_plugin_esm/* getYupInnerErrors */.CJ)(promise.reason);}if(promise.status==='fulfilled'){validations.validIds.push(promise.value.id);}});return _context4.abrupt("return",validations);case 9:case"end":return _context4.stop();}},_callee4);}));return function validateEntriesToPublish(_x5){return _ref6.apply(this,arguments);};}();var handleConfirmPublishAllData=/*#__PURE__*/function(){var _ref7=(0,asyncToGenerator/* default */.Z)(/*#__PURE__*/regenerator_default().mark(function _callee5(selectedEntries){var validations;return regenerator_default().wrap(function _callee5$(_context5){while(1)switch(_context5.prev=_context5.next){case 0:_context5.next=2;return validateEntriesToPublish(selectedEntries);case 2:validations=_context5.sent;if(!Object.values(validations.errors).length){_context5.next=6;break;}toggleNotification({type:'warning',title:{id:'content-manager.listView.validation.errors.title',defaultMessage:'Action required'},message:{id:'content-manager.listView.validation.errors.message',defaultMessage:'Please make sure all fields are valid before publishing (required field, min/max character limit, etc.)'}});throw new Error('Validation error');case 6:return _context5.abrupt("return",bulkPublishMutation.mutateAsync({ids:selectedEntries}));case 7:case"end":return _context5.stop();}},_callee5);}));return function handleConfirmPublishAllData(_x6){return _ref7.apply(this,arguments);};}();var handleConfirmUnpublishAllData=function handleConfirmUnpublishAllData(selectedEntries){return bulkUnpublishMutation.mutateAsync({ids:selectedEntries});};react.useEffect(function(){var CancelToken=axios["default"].CancelToken;var source=CancelToken.source();var shouldSendRequest=canRead;var requestUrl=(0,utils/* getRequestUrl */.IF)("collection-types/".concat(slug).concat(params));if(shouldSendRequest&&requestUrl.includes(requestUrlRef.current)){fetchData(requestUrl,source);}return function(){requestUrlRef.current=slug;source.cancel('Operation canceled by the user.');};},[canRead,getData,slug,params,getDataSucceeded,fetchData]);var defaultHeaderLayoutTitle=formatMessage({id:(0,utils/* getTrad */.OB)('header.name'),defaultMessage:'Content'});var headerLayoutTitle=formatMessage({id:info.displayName,defaultMessage:info.displayName||defaultHeaderLayoutTitle});var _useStrapiApp=(0,helper_plugin_esm/* useStrapiApp */.j1)(),runHookWaterfall=_useStrapiApp.runHookWaterfall;var displayedHeaders=(0,es/* useSelector */.v9)(selectDisplayedHeaders);var tableHeaders=react.useMemo(function(){var headers=runHookWaterfall(exposedHooks/* INJECT_COLUMN_IN_TABLE */.No,{displayedHeaders:displayedHeaders,layout:layout});var formattedHeaders=headers.displayedHeaders.map(function(header){var metadatas=header.metadatas;if(header.fieldSchema.type==='relation'){var sortFieldValue="".concat(header.name,".").concat(header.metadatas.mainField.name);return _objectSpread(_objectSpread({},header),{},{metadatas:_objectSpread(_objectSpread({},metadatas),{},{label:formatMessage({id:(0,utils/* getTrad */.OB)("containers.ListPage.table-headers.".concat(header.name)),defaultMessage:metadatas.label})}),name:sortFieldValue});}return _objectSpread(_objectSpread({},header),{},{metadatas:_objectSpread(_objectSpread({},metadatas),{},{label:formatMessage({id:(0,utils/* getTrad */.OB)("containers.ListPage.table-headers.".concat(header.name)),defaultMessage:metadatas.label})})});});if(!hasDraftAndPublish){return formattedHeaders;}// this should not exist. Ideally we would use registerHook() similar to what has been done
...
var reviewWorkflowColumn=getReviewWorkflowsColumn(layout);if(reviewWorkflowColumn){formattedHeaders.push(reviewWorkflowColumn);}return[].concat((0,toConsumableArray/* default */.Z)(formattedHeaders),[{key:'__published_at_temp_key__',name:'publishedAt',fieldSchema:{type:'custom'},metadatas:{label:formatMessage({id:(0,utils/* getTrad */.OB)("containers.ListPage.table-headers.publishedAt"),defaultMessage:'publishedAt'}),searchable:false,sortable:true},// eslint-disable-next-line react/no-unstable-nested-components
cellFormatter:function cellFormatter(cellData){var isPublished=cellData.publishedAt;var variant=isPublished?'success':'secondary';return/*#__PURE__*/react.createElement(Status/* Status */.q,{width:"min-content",showBullet:false,variant:variant,size:"S"},/*#__PURE__*/react.createElement(Typography/* Typography */.Z,{fontWeight:"bold",textColor:"".concat(variant,"700")},formatMessage({id:(0,utils/* getTrad */.OB)("containers.List.".concat(isPublished?'published':'draft')),defaultMessage:isPublished?'Published':'Draft'})));}}]);},[runHookWaterfall,displayedHeaders,layout,hasDraftAndPublish,formatMessage]);var subtitle=canRead?formatMessage({id:(0,utils/* getTrad */.OB)('pages.ListView.header-subtitle'),defaultMessage:'{number, plural, =0 {# entries} one {# entry} other {# entries}} found'},{number:total}):null;var getCreateAction=function getCreateAction(props){return canCreate?/*#__PURE__*/react.createElement(Button/* Button */.z,(0,esm_extends/* default */.Z)({},props,{forwardedAs:react_router_dom/* Link */.rU,onClick:function onClick(){var trackerProperty=hasDraftAndPublish?{status:'draft'}:{};trackUsageRef.current('willCreateEntry',trackerProperty);},to:{pathname:"".concat(pathname,"/create"),search:query.plugins?pluginsQueryParams:''},startIcon:/*#__PURE__*/react.createElement(Plus/* default */.Z,null),style:{textDecoration:'none'}}),formatMessage({id:(0,utils/* getTrad */.OB)('HeaderLayout.button.label-add-entry'),defaultMessage:'Create new entry'})):null;};return/*#__PURE__*/react.createElement(Main/* Main */.o,{"aria-busy":isLoading},/*#__PURE__*/react.createElement(HeaderLayout/* HeaderLayout */.T,{primaryAction:getCreateAction(),subtitle:subtitle,title:headerLayoutTitle,navigationAction:/*#__PURE__*/react.createElement(helper_plugin_esm/* Link */.rU,{startIcon:/*#__PURE__*/react.createElement(ArrowLeft/* default */.Z,null),to:"/content-manager/"},formatMessage({id:'global.back',defaultMessage:'Back'}))}),!canRead&&/*#__PURE__*/react.createElement(ActionLayout/* ActionLayout */.Z,{endActions:/*#__PURE__*/react.createElement(components_InjectionZone,{area:"contentManager.listView.actions"})}),canRead&&/*#__PURE__*/react.createElement(ActionLayout/* ActionLayout */.Z,{endActions:/*#__PURE__*/react.createElement(react.Fragment,null,/*#__PURE__*/react.createElement(components_InjectionZone,{area:"contentManager.listView.actions"}),/*#__PURE__*/react.createElement(FieldPicker,{layout:layout}),/*#__PURE__*/react.createElement(helper_plugin_esm/* CheckPermissions */.jW,{permissions:permissions.contentManager.collectionTypesConfigurations},/*#__PURE__*/react.createElement(ConfigureLayoutBox,{paddingTop:1,paddingBottom:1},/*#__PURE__*/react.createElement(IconButton/* IconButton */.h,{onClick:function onClick(){trackUsage('willEditListLayout');},forwardedAs:react_router_dom/* Link */.rU,to:{pathname:"".concat(slug,"/configurations/list"),search:pluginsQueryParams},icon:/*#__PURE__*/react.createElement(Cog/* default */.Z,null),label:formatMessage({id:'app.links.configure-view',defaultMessage:'Configure the view'})})))),startActions:/*#__PURE__*/react.createElement(react.Fragment,null,isSearchable&&/*#__PURE__*/react.createElement(helper_plugin_esm/* SearchURLQuery */.m,{label:formatMessage({id:'app.component.search.label',defaultMessage:'Search for {target}'},{target:headerLayoutTitle}),placeholder:formatMessage({id:'global.search',defaultMessage:'Search'}),trackedEvent:"didSearch"}),isFilterable&&/*#__PURE__*/react.createElement(components_AttributeFilter,{contentType:contentType,slug:slug,metadatas:metadatas}))}),/*#__PURE__*/react.createElement(ContentLayout/* ContentLayout */.D,null,canRead?/*#__PURE__*/react.createElement(Flex/* Flex */.k,{gap:4,direction:"column",alignItems:"stretch"},/*#__PURE__*/react.createElement(helper_plugin_esm/* DynamicTable */.tM,{canCreate:canCreate,canDelete:canDelete,canPublish:canPublish,contentTypeName:headerLayoutTitle,onConfirmDelete:handleConfirmDeleteData,onConfirmDeleteAll:handleConfirmDeleteAllData,onConfirmPublishAll:handleConfirmPublishAllData,onConfirmUnpublishAll:handleConfirmUnpublishAllData,isBulkable:isBulkable,isLoading:isLoading// FIXME: remove the layout props drilling
,layout:layout,rows:data,components:{ConfirmDialogDelete:ConfirmDialogDelete,ConfirmDialogDeleteAll:ConfirmDialogDeleteAll_ConfirmDialogDeleteAll},contentType:headerLayoutTitle,action:getCreateAction({variant:'secondary'}),headers:tableHeaders,onOpenDeleteAllModalTrackedEvent:"willBulkDeleteEntries",withBulkActions:true,withMainAction:canDelete&&isBulkable,renderBulkActionsBar:function renderBulkActionsBar(_ref8){var selectedEntries=_ref8.selectedEntries,clearSelectedEntries=_ref8.clearSelectedEntries;return/*#__PURE__*/react.createElement(DynamicTable_BulkActionsBar,{showPublish:canPublish&&hasDraftAndPublish,showDelete:canDelete,onConfirmDeleteAll:handleConfirmDeleteAllData,onConfirmPublishAll:handleConfirmPublishAllData,onConfirmUnpublishAll:handleConfirmUnpublishAllData,selectedEntries:selectedEntries,clearSelectedEntries:clearSelectedEntries});},bulkAction:true},/*#__PURE__*/react.createElement(TableRows,{canCreate:canCreate,canDelete:canDelete,contentType:contentType,headers:tableHeaders,rows:data,withBulkActions:true,withMainAction:canDelete&&isBulkable})),/*#__PURE__*/react.createElement(Flex/* Flex */.k,{alignItems:"flex-end",justifyContent:"space-between"},/*#__PURE__*/react.createElement(helper_plugin_esm/* PageSizeURLQuery */.v4,{trackedEvent:"willChangeNumberOfEntriesPerPage"}),/*#__PURE__*/react.createElement(helper_plugin_esm/* PaginationURLQuery */.tU,{pagination:{pageCount:(pagination===null||pagination===void 0?void 0:pagination.pageCount)||1}}))):/*#__PURE__*/react.createElement(helper_plugin_esm/* NoPermissions */.ZF,null)));}ListView.propTypes={canCreate:(prop_types_default()).bool.isRequired,canDelete:(prop_types_default()).bool.isRequired,canRead:(prop_types_default()).bool.isRequired,canPublish:(prop_types_default()).bool.isRequired,data:(prop_types_default()).array.isRequired,layout:prop_types_default().exact({components:(prop_types_default()).object.isRequired,contentType:prop_types_default().shape({uid:(prop_types_default()).string.isRequired,attributes:(prop_types_default()).object.isRequired,metadatas:(prop_types_default()).object.isRequired,info:prop_types_default().shape({displayName:(prop_types_default()).string.isRequired}).isRequired,layouts:prop_types_default().shape({list:(prop_types_default()).array.isRequired}).isRequired,options:(prop_types_default()).object.isRequired,settings:(prop_types_default()).object.isRequired}).isRequired}).isRequired,isLoading:(prop_types_default()).bool.isRequired,getData:(prop_types_default()).func.isRequired,getDataSucceeded:(prop_types_default()).func.isRequired,pagination:prop_types_default().shape({total:(prop_types_default()).number.isRequired,pageCount:(prop_types_default()).number}).isRequired,slug:(prop_types_default()).string.isRequired};var mapStateToProps=ListView_selectors();function mapDispatchToProps(dispatch){return (0,redux.bindActionCreators)({getData:actions_getData,getDataSucceeded:actions_getDataSucceeded,onChangeListHeaders:onChangeListHeaders,onResetListHeaders:onResetListHeaders},dispatch);}var withConnect=(0,es/* connect */.$j)(mapStateToProps,mapDispatchToProps);/* harmony default export */ const pages_ListView = ((0,redux.compose)(withConnect)(/*#__PURE__*/react.memo(ListView,(isEqual_default()))));
